...
[lat_left_top, lon_left_top] = settings[cname]['coord_top']  # Pittsburgh
[lat_right_bot, lon_right_bot] = settings[cname]['coord_bot']
...
while cnt == 'n':
    ...
    plt.savefig(path_out+'/predicted_vs_real.png')

    cnt=input('Is the first stage predicted image good? (y/n):')
...
if GUF_data:
    ...
    out_dir=path_out+'/predict_GUF'
    grass_union(v1_dir,v2_dir,out_dir)
    
    predict_GUF=gpd.read_file(path_out+'/predict_GUF/predict_GUF.shp')
    ...
    predict_GUF_temp.to_file(path_out+'/predict_GUF_mod')
...
if Building_data =='OSM':
    ...
    for i in np.arange(xtile_min,xtile_max+1):
        print('Xtile '+str(i-xtile_min+1)+' out of '+str(xtile_max-xtile_min))
...
if GUF_data:
    ...
    time.sleep(3)
    v1_dir=path_out+'/predict_GUF_mod/predict_GUF_mod.shp'
    v2_dir=path_out+'/'+Building_data+'_sh_clipped/'+Building_data+'_sh_clipped.shp'
    out_dir=path_out+'/predict_GUF_'+Building_data
    grass_union(v1_dir,v2_dir,out_dir)

    predict_GUF_bld=gpd.read_file(path_out+'/predict_GUF_'+Building_data+'/predict_GUF_'+Building_data+'.shp')
    ...
    predict_GUF_bld_temp.to_file(path_out+'/predict_GUF_'+Building_data+'_mod')
